...
void ResolverArbol(Nodo *ptrActual,Pila *ptrPila);
	...
	while(Eleccion!=4){
		switch(Eleccion){
			...
			case 2:
				...
				break;
			case 3:
				Pila *ptrPilaAux=ptrPila;
				Nodo *ptrActualAux=ptrActual;
				ResolverArbol(ptrActualAux,ptrPilaAux);
				break;
		}
...
void ImprimirVertical(Nodo *ptrRaiz){	//Funcion encargada de imprimir de forma vertical el arbol
	...
	if(ptrRaiz==NULL){					//Si el puntero a la Raiz no apunta a nada este imprimira que el arbol esta vacio.
		...
		OperadorNum(ptrActual->dato); 	//se imprimira el dato y pasara a la siguiente linea y restara en 1 la cantidad de niveles.
		printf("\n");
		NivelMax--;
		while(ptrActual->ptrIzq!=NULL){		//Mientras el puntero Actual tenga un hijo izquierdo, este verificara si tiene hijo derecho, en caso de no tener
			if(ptrActual->ptrDer==NULL){	//se da por sentado que este es el ultimo dato del arbol. 
				OperadorNum(ptrActual->ptrIzq->dato);
				printf("\n");
				break;
			}else{
				if(nivelActual==0){
				...
				OperadorNum(ptrActual->ptrDer->dato);
				for(i=0;i<EspMedio;i++){
					printf(" ");
				}
...
void ResolverArbol(Nodo *ptrActual,Pila *ptrPila){
	if(ptrActual->ptrIzq!=NULL){
		printf("\nArbol Invalido\n");
	}else{
		Nodo *ptrPadre=EliminarPila(&ptrPila);
		if(ptrPadre->ptrIzq==ptrActual){
			int aux=0;
			int a=(int)(ptrPadre->ptrIzq)->dato-48;
			int b=(int)(ptrPadre->ptrDer)->dato-48;
			switch(ptrPadre->dato){
				case '+':
					aux=a + b;
					ptrPadre->dato=aux;
					printf("%d\n",aux);
...
void EncontrarAnterior(Nodo **ptrActual,Pila **ptrPila){	//Funcion que se encarga de reubicar el puntero actual al nodo en el cual debe ingresarse el siguiente dato.
